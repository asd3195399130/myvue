<template>
  <div>
      ...
      <el-table-column fixed="right" label="操作" width="300">
        <template slot-scope="scope">
          <el-button @click="handleClick(scope.row.id)" type="text" size="small"
            >查看</el-button
          >
          <el-button type="text" size="small" @click="editclick(scope.$index)"
            >编辑</el-button
          >
          <el-button type="text" size="small" @click="del(scope.$index)"
            >删除</el-button
          >
        </template>
      </el-table-column>
    ...
    <div>
      <ul>
        <li v-for="item in succ" :key="item">{{ item }}</li>
      </ul>
    </div>
  </div>
</template>
...
export default {
  methods: {
    ...
    submitForm() {
      ...
      (this.text = "点我修改"), (this.drawer = false);
    },
    resetForm(formName) {
  ...
  watch: {
    searchName(newVal) {
      ...
      if (this.searchName == "") {
        ...
        return (this.tableData = this.tableData.filter((item) =>
